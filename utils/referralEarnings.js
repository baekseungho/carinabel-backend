...
async function distributeReferralEarnings(buyer, purchaseAmount, isFirstPurchase) {
    ...
    // ❌ 일반회원이면 수당 지급 불가
    const eligibleLevels = ["IPC회원", "대리점", "총판"];
    if (!eligibleLevels.includes(referrer.membershipLevel)) {
        console.log(`⚠️ 수당 미지급 - 추천인 등급이 낮음: ${referrer.email} (${referrer.membershipLevel})`);
        return;
    }
    ...
    console.log(`✅ 추천인 수당 지급 완료: ${commission}원 (${referrer.email})`);
}
